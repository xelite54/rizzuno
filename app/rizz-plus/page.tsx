  ...
  return (
    <main className={`${styles.page} h-dvh overflow-y-auto text-[#f5eff5]`}>
      <nav className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4 sm:px-10">
        ...
        <Link href={returnTo} className="rounded-full border border-white/15 px-4 py-2 text-[13px] text-white/70 hover:bg-white/5">← Back</Link>
      </nav>
      <div className={styles.layout}>
        ...
        <section className={styles.details} aria-labelledby="membership-heading">
          ...
          {active && <Link href={returnTo} className="mt-3 block rounded-xl border border-white/15 px-4 py-3 text-center text-[13px] text-[#e8cedf] hover:bg-white/5">← Back to where you left off</Link>}
          ...
          <p className="mt-3 text-[11px] leading-relaxed text-white/60">Temporary test access: Get Rizz+ unlocks the features for your signed-in account without payment. No card required and no automatic charge from this activation. Paid checkout is not enabled for new memberships yet.</p>
