...
/**
 * The large avatar at the top of another user's profile — their photo (tap
 * to enlarge) or the initial-letter fallback. A photo that fails to load
 * (e.g. /api/media answering 403/404/503) also falls back to the initial
...
export function ProfileAvatar({ photo, identity }: { photo: string | null; identity: string }) {
  ...
  if (!photo || failedSrc === photo) {
    return (
      <span className="flex h-24 w-24 items-center justify-center overflow-hidden rounded-full bg-accent-2 text-[32px] font-semibold text-accent-foreground">
  ...
  return (
    <>
      ...
      >
        ...
        <img src={photo} alt="" onError={() => { setFailedSrc(photo); setEnlarged(false) }} className="h-full w-full rounded-full object-cover" />
