...
- Complete [US_STATE_LAUNCH_MATRIX.md](US_STATE_LAUNCH_MATRIX.md). Choose nationwide review or a limited `SUPPORTED_US_REGIONS` allowlist, then set `US_STATE_LAUNCH_REVIEW_APPROVED=true` only after that operator review. No state is pre-approved by the repository.
...
`SUPPORTED_COUNTRIES` is a comma-separated list of uppercase country codes, validated by `lib/country.ts` (no `*`). `SUPPORTED_US_REGIONS` optionally narrows U.S. access to reviewed 50-state/DC codes. `LAUNCH_GEO_SOURCE=vercel` selects the currently supported trust boundary. The web deployment must actually run behind Vercel, which supplies `x-vercel-ip-country` and `x-vercel-ip-country-region`; direct/untrusted proxy deployments are not supported for public rollout. When state gating is enabled, unknown or unsupported states fail closed. Unknown or denied countries get HTTP 451 on product and sign-in requests. Public legal/contact pages remain reachable. Ticket issuance checks the same allowlist; the signed country claim is checked during WebSocket hello. Use identical configuration/revision on web and realtime. Direct WS access cannot mint a valid ticket.
