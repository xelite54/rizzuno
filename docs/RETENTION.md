...
| cybertiplineCases | Restricted operator-entered U.S. case decision, receipt and preservation state. An ordinary underage report creates no row. A manually recorded submission requires a receipt and at least one year of scoped preservation; a longer active legal hold controls. |
...
A CyberTipline preservation is scoped to the linked report, its evidence/history and safety/moderation context, and its referenced match session. It begins only after an authorized reviewer records a manual submission timestamp and receipt/reference. The database refuses an expiration earlier than one year after that timestamp and refuses to shorten or release an unexpired preservation. This does not create evidence: calls remain unrecorded, automatic screenshots and continuous video analysis remain disabled, and there is no video evidence store. The broader legal-hold mechanism can preserve more records or extend beyond that date when separately directed.
