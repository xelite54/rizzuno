...
Latest recovery verification (2026-09-13): `MEDIA_RETRY_TEST=1 npm run test:webrtc:browser` passed all browser assertions with a 120,310 ms call hold and 24 samples. Both clients recovered automatically after dropping their first two search requests. Added checks confirm that brief waiting/pause events do not flash Connecting. Same-peer transport recovery, camera replacement, skip/new match, direct calls, failed-playback cleanup, and sign-out passed. The retry scenario is optional; the normal runner retains the default two-minute stability hold. These are simulated Chromium devices, not a physical phone/network acceptance test.
...
The live TURN probe in `verifyProductionTurn.mts` now accepts only an operator-supplied short-lived credential exported from the authenticated endpoint, with explicit host allowlisting and opt-in. It does not retrieve cloud tokens or permanent shared secrets. It fails unless relay candidates are allocated, both selected candidates are relay, and data ping/pong succeeds. See `docs/WEBRTC_ACCEPTANCE.md`. Production relay allocation remains unverified in this hardening session.
