...
test("a delayed find retry cannot end an established random match", async () => {
  ...
  try {
    ...
    b.send({ type: "signal", roomId: matched.roomId, data: { kind: "ice-restart-request", negotiationId: "n1" } })
    assert.equal((await a.waitForType("signal")).data.kind, "ice-restart-request")
    a.send({ type: "signal", roomId: matched.roomId, data: { kind: "offer", sdp: "restart-offer", negotiationId: "n1" } })
    assert.deepEqual((await b.waitForType("signal")).data, { kind: "offer", sdp: "restart-offer", negotiationId: "n1" })
    b.send({ type: "signal", roomId: matched.roomId, data: { kind: "answer", sdp: "restart-answer", negotiationId: "n1" } })
    assert.deepEqual((await a.waitForType("signal")).data, { kind: "answer", sdp: "restart-answer", negotiationId: "n1" })
    a.send({ type: "skip" })
