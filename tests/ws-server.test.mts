...
test("realtime rejects blocked usernames on hello and profile changes", async () => {
  ...
  try {
    ...
    const a = await connectAndHello(server.url, filteredId, { gender: "male", username: "N.1.G.G.3.R" })
    ...
    dbMockState.usernames.set(filteredId, "blue.sky") // Successful HTTP profile update precedes notification.
...
test("a delayed find retry cannot end an established random match", async () => {
  ...
  try {
    ...
    const c = await connectAndHello(server.url, "stable-find-c", { gender: "female" })
    c.send({ type: "find" })
    await c.waitForType("queued")
    a.send({ type: "find" }); b.send({ type: "find" })
    ...
    a.send({ type: "signal", roomId: matched.roomId, data: { kind: "offer", sdp: "restart-offer" } })
    assert.deepEqual((await b.waitForType("signal")).data, { kind: "offer", sdp: "restart-offer" })
    b.send({ type: "signal", roomId: matched.roomId, data: { kind: "answer", sdp: "restart-answer" } })
    assert.deepEqual((await a.waitForType("signal")).data, { kind: "answer", sdp: "restart-answer" })
    a.send({ type: "skip" })
